...
function validateConfirm() {
    clearErrors();
    var returnval = true;
    var passworddat = document.getElementById('forgot_email').value;
    var cpassworddat = document.getElementById('forgot_pwd').value;
    ...
    if (cpassworddat.length == 0) {
        seterror("box2", "required");
        returnval = false;
    }

    if (passworddat.length > 8 && passworddat.length < 15 && cpassworddat != passworddat) {
        seterror("box2", "Password and Confirm Password not match");
        returnval = false;
    }
    return returnval;
}
...
function validateOtp() {
    clearErrors();
    var returnval = true;
    var otp = document.getElementById('forgot_email').value;

    if (otp.length == 0) {
        $.notify("Please Fill OTP", {
            globalPosition: 'bottom right',
            className: 'error'
        });
        returnval = false;
...
function validateEmail() {
    ...
    if (email.length == 0) {
        $.notify("Please Fill Valid Email", {
            globalPosition: 'bottom right',
            className: 'error'
        });
        returnval = false;
